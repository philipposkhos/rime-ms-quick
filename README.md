...
共用步驟：把檔案放到同文「用戶資料夾」根目錄（舊版常見路徑是 `/sdcard/rime`；3.3 請到同文設定查看「用戶資料夾」），勾選方案「傳統速成」，再部署。若要簡體排位與繁體一致，請另外複製 **`Android/opencc/`**（不要用根目錄那份 `opencc/`），詳見下方「繁簡輸入位置不同」。
...
這是將本來 opencc 中繁轉簡「一對多」都改成「一對一」，簡體模式下速成候選排位才會跟繁體一致。請放進手機／電腦**現有**的 `opencc` 資料夾，不要整夾覆蓋。

電腦和 Android **不是同一套檔**，請依平台選資料夾：

| 平台 | 專案檔案位置 | 目標位置 |
| --- | --- | --- |
| Windows 小狼毫 | 根目錄 `opencc/`（`t2s.json`、`TSCharacters.ocd2`） | `\data\opencc\` |
| Android 同文（3.1.3／3.3 皆此） | **`Android/opencc/`**（`t2s.json`、`TSCharacters.txt`） | 用戶資料夾底下的 `opencc/` |
